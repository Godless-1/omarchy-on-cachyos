...
| The new duplicate-entry check reported a non-blocking fault as a failure | A failure makes the verdict print `DO NOT REBOOT`, directly above the finding's own text saying booting was fine. Seen on the first machine it ran on, in the same output | Downgraded to a warning; a test reads the branch and fails if it calls `bad()` again |
| `omarchy-window` could never start on a host without `qdbus` | `QDBUS=$(for q in ...; do command -v "$q" ... done)` takes the loop's status, which is the last failed lookup when none is installed; `set -e` then ended the script on that line, before printing anything - while further down it offered advice headed "Other desktops:" for exactly those users. Only surfaced when a suite that had only ever run on a KDE machine ran in CI | `|| true` on the assignment; every use already guarded on `-n ${QDBUS:-}` |
| 23 files of makepkg's bare clone were committed, and the check that said otherwise was wrong | `git add -A` staged it before `.gitignore` covered it. Asked whether that could happen, `git add -A --dry-run` was used and listed nothing - **because the files were already tracked**, and `add` reports only what it would newly stage. The operator was told the concern was unfounded. `git ls-files` was the right question and would have said 23. It surfaced by breaking `reuse lint` | Untracked and ignored; `test-packaging.sh` now fails on any tracked path that looks like build output, and refuses to pass when it cannot read the repository |
